...
for kk = 1:length(hiddenLayerSize)
    ...
    M = 2 * windowSize;  % feature input (vector) size
    
    
    % name of data file (donor and acceptor sequences)
    nameP = 'Acceptor_Train_Positive';
    ...
    % combine Positive and Negative class
    combinedTrain = [seqsP; seqsN];
    
    % shuffle Positive and Negative class
    ...
    % tansig + tansig: YES, hidden = 10, 76.9, 75.5; 67.8, 86.5; 76, 81; 76.4,
    % 78.3; 67.3, 78.7
    % logsig + tansig: YES, hidden = 10; 69.2, 80.9; 78.8, 85.4; 78.4, 82.3;
    % 73.6, 79.3; 79.3, 82.9
    % logsig + purelin: YES, hidden = 10; 76.4, 82; 73.6, 85; 75, 83.4; 76,
    % 79.8; 76, 82.7
    % tansig + purelin: YES, hidden = 10, 76.4, 78.3; 76.4, 82.4; 79.3, 85.1; 73.1, 82.6; 76.4, 84.6
    % hidden = 40, 84.1, 82.9;
    % tansig + logsig: NO
    % tansig + hardlim: NO
    % tansig + softmax: NO
    ...
    for i = 1:3
        ...
        if (i==3)
            ...
            name = strcat('Confusion_Acceptor_Training', '_', num2str(M), '_', num2str(hiddenLayerSize(kk)), '_', net.layers{1}.transferFcn, '_', net.layers{2}.transferFcn, '_', date);
            %figure, plotconfusion(targetsTrain,outputsTrain)
            %title('Training');
            %set(gcf, 'PaperPositionMode', 'auto');
            %print('-depsc2', strcat(name,'.eps'));
            
            fid=fopen('Running Time Acceptor.txt', 'a');
            fprintf(fid, '\r\n'); 
            fprintf(fid, '%s\r\n', datetime);
        ...
        % Testing
        % name of data file (donor and acceptor sequences)
        nameTestP = 'Acceptor_Test_Positive';
        ...
        nameTestN = 'Acceptor_Test_Negative';
        ...
        if (i==3)
            ...
            name = strcat('Confusion_Acceptor_Testing', '_', num2str(M), '_', num2str(hiddenLayerSize(kk)), '_', net.layers{1}.transferFcn, '_', net.layers{2}.transferFcn, '_', date);
            figure, plotconfusion(targetsTest,outputsTest)
            title('Testing');
            set(gcf, 'PaperPositionMode', 'auto');
            print('-depsc2', strcat(name,'.eps'));
            
            fid1=fopen('Running Time Acceptor.txt', 'a');
