...
[X_test,Y_test,T_test,auROC_test,OPTROCPT_test] = perfcurve(target,score,1);
            %fprintf(fid1, 'Test auROC: %f\r\n', auROC_test);
            
            % Draw Precision-Recall (auPRC)
            [Xpr_test,Ypr_test,Tpr_test,auPRC_test] = perfcurve(target,score,1, 'xCrit', 'reca', 'yCrit', 'prec'); %
            %figure
...
if (plot_pr || plot_roc)
    ...
    if (~hold_fig)
        ...
        if (plot_roc)
            ...
            title(strcat('ROC curve (auROC: ', num2str(auROC_test), ')'));
            %axis([0 1 0 1]);
            if (plot_roc && plot_pr)
                % double the width
                rect = get(gcf,'pos');
        ...
        if (plot_pr)
            ...
            title(strcat('Precision-Recall curve (auPRC: ', num2str(auPRC_test), ')'));
        end
